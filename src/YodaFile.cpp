...
void YodaFile::comparePlotsTakeSmaller(YodaFile &rhs) {
	...
	for (int i = 0; i < this->getNumberOfPlots(); i++) { //Test with 3 plots first
		...
		for (int k = 0; k < this->arrPlots[i]->getNumberOfBins(); k++) {
			file << scientific << this->arrPlots[i]->values[k][0] << "\t" << this->arrPlots[i]->values[k][1] << "\t";
			for (int l = 2; l < 6; l++) { //only columns 2-3-4-5 change, but its easier to compare all for printing reasons
			...
			temp4 = this->arrPlots[i]->values[k][6];
			file << temp4 << endl;
...
void YodaFile::showStatistics(YodaFile& rhs) {
	long double temp1, temp2, temp3;
	...
	for (int i = 0; i < this->getNumberOfPlots(); i++) {
		...
		cout << "In " << this->myName << " ,plot: " << i + 1 << " : there are "
				<< bigger << "bigger values than in " << rhs.myName << ", "
				<< endl;
		cout << "there are " << smaller << "smaller values, and " << equal
				<< " equal values. " << endl << endl;
	}
}

void YodaFile::switchHeaders(YodaFile& rhs) {
	long double temp1;
	ofstream file("./newFiles/switchHeaders.yoda");
	string* a;

	for (int i = 0; i < this->getNumberOfPlots(); i++) { //Test with 3 plots first
		a = rhs.arrPlots[i]->getHeader();
		for (int h = 0; h < 14; h++) {
			file << a[h] << endl;
		}
		for (int k = 0; k < this->arrPlots[i]->getNumberOfBins(); k++) {
